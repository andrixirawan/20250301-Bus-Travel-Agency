      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold">Daftar Kota</h2>
                        ...
                        <tr>
                          ...
                          <td class="px-6 py-4 text-sm text-gray-800">
                            <ul class="list-disc list-inside">
                              @foreach ($city->tourist_attractions as $attraction)
                                <li>{{ $attraction }}</li>
                              @endforeach
                            </ul>
                          </td>
                          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <form action="{{ route('cities.destroy', $city) }}" method="POST"
                              ...
                              <button type="submit" class="text-red-600 hover:text-red-900"
                                onclick="return confirm('Apakah Anda yakin ingin menghapus kota ini?')">Hapus</button>
                            </form>
                          </td>
                        </tr>
                      @endforeach
  ...
  @push('scripts')
    <script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        const dataTable = new HSDataTable('#cityTable');
